...
public:
  ...
  void onVREvent(const VRDataIndex &event)
  {
    ...
    if (event.getName() == "Wand0_Move" || event.getName() == rcontrol + "_Move")
    { // Set up for Wand Movement
    ...
    else if (event.getName() == "Kbd5_Down")
    { // instacrashes :( || event.getName() == "Mouse_Left_Click_Down"){
      //_pm->SetShaders();
    }
    ...
    else if (event.getName() == "KbdV_Down")
    { // || ((event.getName() == rcontrol + "_Axis0Button_Down") && trackPadActive("right") &&(right_trackPad0_X*right_trackPad0_X) + (right_trackPad0_Y*right_trackPad0_Y) < (0.33*0.33))) {
      _fmv.showOldModel = !_fmv.showOldModel;
    }
    ...
    if (_horizAngle > 6.283185)
      _horizAngle -= 6.283185;
    if (_horizAngle < 0.0)
      _horizAngle += 6.283185;

    if (_vertAngle > 6.283185)
      _vertAngle -= 6.283185;
    if (_vertAngle < 0.0)
      _vertAngle += 6.283185;
  }
